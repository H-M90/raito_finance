...
const focusableElements = root => [...root.querySelectorAll('a[href], button, input, select, textarea, [tabindex]')]
...
const syncOverlayScroll = () => {
    const locked = (mobileNavigation.matches && sidebar?.classList.contains('open')) || document.querySelector('.modal.open');
...
const setSidebarOpen = (open, restoreFocus = true) => {
    ...
    if (mobileNavigation.matches) sidebar.setAttribute('aria-hidden', String(!open));
    ...
    syncOverlayScroll();
    if (open) focusableElements(sidebar)[0]?.focus();
    else if (restoreFocus && sidebarOpener?.getClientRects().length) sidebarOpener.focus();
};
sidebarToggles.forEach(toggle => toggle.addEventListener('click', () => {
    const open = !sidebar?.classList.contains('open');
    if (open) sidebarOpener = toggle;
    setSidebarOpen(open);
}));
mobileNavigation.addEventListener('change', () => setSidebarOpen(false, false));
...
document.querySelectorAll('.form-group').forEach((group, index) => {
    ...
    if (!label || !field || label.htmlFor || label.contains(field)) return;
    if (!field.id) field.id = `ui-field-${index}`;
    label.htmlFor = field.id;
});

// Observe existing dialog visibility so each workflow retains its own open/close logic.
let lastPageFocus = document.activeElement;
document.addEventListener('focusin', event => {
    if (!event.target.closest('.modal.open')) lastPageFocus = event.target;
});
document.querySelectorAll('.modal').forEach((modal, index) => {
    ...
    if (heading && !dialog.hasAttribute('aria-labelledby')) {
        if (!heading.id) heading.id = `shared-dialog-title-${index}`;
    ...
    const syncDialog = () => {
        ...
        if (open && !wasOpen) {
            returnFocus = modal.contains(document.activeElement) ? lastPageFocus : document.activeElement;
            if (!modal.contains(document.activeElement)) (focusableElements(dialog)[0] || dialog).focus();
        } else if (!open && wasOpen && returnFocus?.isConnected && !document.querySelector('.modal.open')) {
            returnFocus.focus();
        }
        wasOpen = open;
        syncOverlayScroll();
...
document.addEventListener('keydown', event => {
    ...
    const panel = modal || (mobileNavigation.matches && sidebar?.classList.contains('open') ? sidebar : null);
    if (event.key === 'Escape') {
        if (document.querySelector('.select2-container--open')) return;
        if (modal && !modal.matches('[data-confirm-modal]')) modal.classList.remove('open');
        else if (!modal && panel) setSidebarOpen(false);
        document.querySelectorAll('.quick-actions-menu[open]').forEach(menu => {
            menu.open = false;
    ...
    if (!first) { event.preventDefault(); (panel.querySelector('.modal-card') || panel).focus(); }
