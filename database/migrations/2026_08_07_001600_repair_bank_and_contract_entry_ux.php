...
return new class extends Migration {
    ...
    {
        // A database can contain migration history from a previous project copy
        // while the bank reconciliation tables themselves are missing. This
        // migration deliberately re-checks the physical schema instead of
        // trusting the migrations table.
        ...
        if (! Schema::hasTable('bank_statement_rows')) {
            Schema::create('bank_statement_rows', function (Blueprint $table) {
                $table->id();
                $table->foreignId('bank_statement_batch_id')->constrained('bank_statement_batches')->cascadeOnDelete();
        ...
        if (Schema::hasTable('contracts') && ! Schema::hasColumn('contracts', 'domain')) {
            Schema::table('contracts', function (Blueprint $table) {
                $table->string('domain')->nullable()->after('activity_type');
            });
        }

        if (Schema::hasTable('contract_station')) {
            if (! Schema::hasColumn('contract_station', 'pts_count')) {
                Schema::table('contract_station', fn (Blueprint $table) => $table->unsignedInteger('pts_count')->default(0)->after('station_id'));
            }
            if (! Schema::hasColumn('contract_station', 'sensor_count')) {
                Schema::table('contract_station', fn (Blueprint $table) => $table->unsignedInteger('sensor_count')->default(0)->after('pts_count'));
            }
        }
    }
    ...
    {
        // Deliberately non-destructive: this is a production repair migration.
        // It may have adopted columns/tables that already existed before this
        // migration record, so rollback must never remove business data.
    }
};
